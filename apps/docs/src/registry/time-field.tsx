...
/**
 * A labelled time field, ready to drop into a form.
 *
 * Distributed through the registry at /r/time-field.json, so `shadcn add` copies
  ...
  return (
    <div className="rx-field" data-invalid={error ? '' : undefined}>
      {children ? (
        ...
        // from this one, and the hour segment is where entry starts.
        <label className="rx-field__label" htmlFor={`${fieldId}-hour`}>
          {children}
        </label>
      ) : null}
