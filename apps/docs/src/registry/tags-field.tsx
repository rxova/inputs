...
/**
 * A labelled tags field, ready to drop into a form.
 *
 * Distributed through the registry at /r/tags-field.json, so `shadcn add` copies
...
export function TagsInputField({
  ...
  return (
    <div className="rx-field" data-invalid={error ? '' : undefined}>
      {children ? (
        // A real `<label htmlFor>`: the entry box is a real `<input>`, whose id is
